...
├── data/
│   ├── raw/
│   │   └── origin_data.xlsx           ← original Kaggle dataset
│   ├── interim/
│   │   └── data_cleaned.xlsx          ← after Process phase
│   └── processed/
│       └── analysis_ready.csv         ← final analysis-ready dataset (996 rows)
│
├── docs/
│   ├── data_dictionary.md             ← column descriptions & encoding assumptions
│   ├── project_log.docx               ← decision log across all 6 phases
│   └── kaggle_author_inquiry.png      ← screenshot of message to dataset author
│
├── sql/
...
│
├── excel/
│   └── eda_workbook.xlsx              ← pivot tables, descriptive stats
│
├── tableau/
│   ├── dropout_dashboard.twbx         ← packaged Tableau workbook
│   └── screenshots/                   ← dashboard previews for README
│
└── reports/
    └── case_study.md                  ← final narrative write-up
```
...
Full guiding questions and decision rationale: [project_log.docx](docs/project_log.docx).
...
- **Outreach to author:** Kaggle discussion topic posted requesting encoding details and feature-engineering formulas ([screenshot](docs/kaggle_author_inquiry.png))
...
- Dropped rows with missing target values → **996 rows** in the analysis-ready dataset
- Imputed remaining missing values (median for numeric / mode for categorical) with `was_missing_*` audit flags
- Recomputed engineered features with denominator guards to remove divide-by-near-zero outliers (originals preserved in `data/raw/` for transparency)
...
**Case study write-up:** [reports/case_study.md](reports/case_study.md)
...
- Document encoding assumptions when a data dictionary is missing, and attempt author contact (done here — see [docs/kaggle_author_inquiry.png](docs/kaggle_author_inquiry.png))
...
4. Engineered features with severe outliers were recomputed; original values preserved in raw data for transparency.
